...
## Экспериментальная нейросеть длинных нард

Отдельная сеть оценки позиций обучается офлайн в самоигре и проверяется на независимых
партиях. В лобби доступна как отдельный экспериментальный **«Сложный бот-нейро»**:
«Играть с ботом» → «Длинные нарды» → «Сложный бот-нейро».
Модель `hard-neuro-448-v1` обучена на 448 партиях, её веса во время игры не меняются.
Обычный сложный бот сохранён; нейросеть не связана с генерацией кубиков.
Обучение, результаты проверки и ограничения описаны в
[документации](docs/long-bot-neural.md).

```bash
npm run demo:long-bot-neural
```

Локальный стенд: `http://127.0.0.1:3909/experiments/long-neural/index.html`.
Он принимает JSON-модель и позволяет проверить её законные ходы на учебной доске.
...
- Стартовый бросок по одному кубику для каждой стороны определяет очерёдность; победитель затем отдельно бросает два игровых кубика для первого хода.
