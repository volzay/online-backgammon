...
В лобби отдельным пунктом доступен **«Сложный бот-нейро»**:
«Играть с ботом» → «Длинные нарды» → «Сложный бот-нейро». Сейчас это
замороженный кандидат V2 `hard-neuro-search-v2-32games-v1` в режиме
тестирования игроками. Он использует историческую модель как начальную точку
и 32 новые завершённые учебные партии, но его development-проверка пока
неполна: завершено 5 из 6 запрошенных партий, одна попытка прервана.
Это не подтверждает процент побед против игроков и не означает достижения
цели 50% или 65%.

Во время партии веса не изменяются. Обычный сложный бот остаётся отдельным,
его память и обучение не смешиваются с нейроботом. Реальные кубики создаёт
тот же независимый защищённый сервис бросков, а не модель выбора ходов.
В опубликованную модель не входят приватный учебный журнал, архивы партий,
имена игроков, приватные seed кубиков/обучающих потоков или ключи. Публичный
`model.seed` — только воспроизводимый параметр инициализации сети и входит в
отпечаток модели. Архитектура, ограничения и точные счётчики
описаны в [документации V2](docs/long-bot-neural-v2.md).
