...
(function (root) {
  ...
  function rollControls(item, { lang = language(), context } = {}) {
    try {
      ...
      const source = signed && item.fairDiceProof && typeof item.fairDiceProof === 'object'
        ? `<p>${lang === 'en' ? 'Recorded source' : 'Сохранённый источник'}: drand quicknet · ${lang === 'en' ? 'round' : 'раунд'} ${escape(item.fairDiceProof.beacon?.round ?? '')}</p><p>${lang === 'en' ? 'Chain' : 'Цепочка'}: <code>${escape(item.fairDiceProof.chainHash || '')}</code></p><p>${lang === 'en' ? 'Source signature' : 'Подпись источника'}:</p><pre>${escape(item.fairDiceProof.beacon?.signature || '')}</pre>` : '';
  ...
  function gameSummary(result, lang) {
    ...
    return heading + details + verifiedSources + (legacy
      ...
      : lang === 'en' ? ' Players and bots use the same independent drand source.' : ' Игроки и боты используют один независимый источник drand.')
  ...
  async function run(button, wholeGame) {
    ...
    try {
      ...
      const signed = wholeGame ? Boolean(result.sourceCounts) : ['drand', 'drand-quicknet-v1'].includes(result.protocol);
      const fullSource = wholeGame ? result.sourceCounts && result.sourceCounts.signed === result.counts.rolls
        && result.sourceCounts.sourceVerified === result.counts.rolls && result.sourceCounts.reservationVerified === result.counts.rolls
        && result.counts.rolls > 0 && result.counts.diceVerified === result.counts.rolls
        : result.sourceVerified && result.reservationVerified && result.diceStatus === 'verified';
