...
const SUPPORTED_PROTOCOLS = Object.freeze([FairDice.PROTOCOL, SYSTEM_PROTOCOL]);
...
const SAFE_CODES = new Set(['INVALID_ROOM_CODE', 'INVALID_REQUEST_ID', 'INVALID_CREDENTIAL', 'PLAYER_AUTH_REQUIRED',
  ...
  'INVALID_CLIENT_SEED', 'FAIR_PROTOCOL_INVALID', 'FAIR_CLIENT_SEED_MISMATCH',
...
function protocolOf(request) {
  requireThat(request && typeof request === 'object'
    && Object.hasOwn(request, 'commitment') !== Object.hasOwn(request, 'round'), 'FAIR_PROTOCOL_INVALID', 422);
  return Object.hasOwn(request, 'commitment') ? SYSTEM_PROTOCOL : FairDice.PROTOCOL;
}

function roomProtocol(room) { return room.fairDiceProtocol ?? FairDice.PROTOCOL; }

function compactStateAck(room, result = {}, flags = {}) {
  const response = {
    ok: true,
    version: Number.isSafeInteger(result?.version) ? result.version : room.version,
    gameId: typeof result?.gameId === 'string' && UUID.test(result.gameId) ? result.gameId : room.gameId,
    variant: room.variant,
    protocol: roomProtocol(room),
    ...flags,
  };
  // Store results are privileged and may contain the complete game_state.
  // Only this fixed acknowledgement crosses the public service boundary.
  for (const flag of ['unchanged', 'deferred']) {
    if (result?.[flag] === true) response[flag] = true;
  }
  return response;
}
...
function createFairDiceService({ store, signingKey, fetchImpl = globalThis.fetch,
  ...
  function startJob(request) {
    requireThat(protocolOf(request) === FairDice.PROTOCOL, 'FAIR_PROTOCOL_INVALID', 422);
  ...
  async function getAuthorizedRoom(code, headers) {
    ...
    requireThat(room?.roomCode === code && room.fairDiceRequired === true && UUID.test(room.gameId)
      ...
      && SUPPORTED_PROTOCOLS.includes(roomProtocol(room)), 'FAIR_ROOM_REQUIRED', 422);
    return room;
  }

  function binding(request, room) {
    requireThat(request?.roomCode === room.roomCode && request.gameId === room.gameId
      && request.variant === room.variant && protocolOf(request) === roomProtocol(room), 'REQUEST_CONTEXT_MISMATCH', 403);
  }

  function liveSystemBinding(recorded, room) {
    binding(recorded?.request, room);
    requireThat(protocolOf(recorded.request) === SYSTEM_PROTOCOL
      && (!recorded.protocol || recorded.protocol === SYSTEM_PROTOCOL), 'FAIR_PROTOCOL_INVALID', 422);
    requireThat(!recorded.cancelled, 'DICE_REQUEST_CANCELLED', 409);
    requireThat(!recorded.consumed && room.pending?.request?.id === recorded.request.id
      && recorded.request.positionHash === room.serverPositionHash
      && room.status === 'joined' && room.state && !room.state.winner,
    'PROOF_NOT_OUTSTANDING', 409);
    const opening = recorded.request.label === 'opening';
    requireThat(opening ? room.state.phase === 'opening' && recorded.request.color === 'none'
      : recorded.request.label === 'roll' && room.state.phase === 'roll'
        && recorded.request.color === room.state.turn, 'REQUEST_CONTEXT_MISMATCH', 409);
    requireThat(opening ? room.actor.ownsHost : recorded.request.color === room.actor.actorColor
      || (room.actor.bot && room.actor.ownsHost), 'UNVALIDATED_ACTOR', 403);
  }

  async function completeSystemRequest(request) {
    if (jobs.has(request.id)) {
      requireThat(same(jobs.get(request.id).request, request), 'REQUEST_CONTEXT_MISMATCH', 409);
      return jobs.get(request.id).promise;
    }
    ...
    job.promise = (async () => {
      const recorded = await store.getRequest(request.id);
      requireThat(recorded?.request && same(recorded.request, request)
        && (!recorded.protocol || recorded.protocol === SYSTEM_PROTOCOL), 'REQUEST_CONTEXT_MISMATCH', 409);
      requireThat(!recorded.cancelled && !recorded.consumed, 'DICE_REQUEST_CANCELLED', 409);
      requireThat(typeof recorded.clientSeed === 'string' && HEX_32.test(recorded.clientSeed)
        && typeof recorded.privateSeed === 'string' && HEX_32.test(recorded.privateSeed), 'INVALID_CLIENT_SEED', 409);
      if (recorded.proof) {
        await FairDice.verifyProof(recorded.proof, { publicKey, context: request });
        requireThat(recorded.proof.commitReveal?.clientSeed === recorded.clientSeed,
          'FAIR_CLIENT_SEED_MISMATCH', 409);
        return recorded.proof;
      }
      // The database fixed both seeds before this function. No RNG, alternate
      // client seed, bot choice or source fallback is reachable on retries.
      const proof = await FairDice.createSystemProof(FairDice.signReservation(request, signingKey),
        recorded.privateSeed, recorded.clientSeed);
      try { await store.commitProof(request.id, proof); }
      catch (failure) {
        // A lost commit reply is recovered by reading the same ledger row;
        // failure to read/commit leaves this request pending, never rerolled.
        const recovered = await store.getRequest(request.id);
        if (!recovered?.proof) throw failure;
        requireThat(same(recovered.proof, proof) && !recovered.cancelled,
          'REQUEST_CONTEXT_MISMATCH', 409);
      }
      return proof;
  ...
  async function reserve(body, headers) {
    ...
    const receipt = FairDice.signReservation(request, signingKey);
    if (protocolOf(request) === SYSTEM_PROTOCOL) {
      const recorded = await store.getRequest(request.id);
      requireThat(recorded?.request && same(recorded.request, request), 'REQUEST_CONTEXT_MISMATCH', 409);
      requireThat(recorded.clientSeed == null || typeof recorded.clientSeed === 'string'
        && HEX_32.test(recorded.clientSeed), 'INVALID_CLIENT_SEED', 503);
      // The accepted first client contribution may be public for reconnects;
      // the private server seed is deliberately absent until a ready proof.
      return { status: 202, body: { status: 'pending', ...receipt, receipt, clientSeed: recorded.clientSeed || null } };
    }
    startJob(request);
    return { status: 202, body: { status: 'pending', ...receipt, receipt } };
  }

  async function challenge(body, headers) {
    exactFields(body, ['code', 'requestId', 'requestHash', 'clientSeed']);
    requireThat(typeof body.clientSeed === 'string' && HEX_32.test(body.clientSeed), 'INVALID_CLIENT_SEED', 400);
    requireThat(typeof body.requestHash === 'string' && HEX_32.test(body.requestHash), 'INVALID_FIELDS', 400);
    ...
    liveSystemBinding(recorded, room);
    requireThat(FairDice.systemRequestHash(recorded.request) === body.requestHash,
      'REQUEST_CONTEXT_MISMATCH', 409);
    requireThat(typeof store.acceptClientSeed === 'function', 'INVALID_STORE', 503);
    const accepted = await store.acceptClientSeed(id, body.clientSeed, headers);
    requireThat(accepted?.request && same(accepted.request, recorded.request)
      && !accepted.cancelled && !accepted.consumed, 'REQUEST_CONTEXT_MISMATCH', 409);
    // Compare against the persisted first seed even when another challenge won
    // the lock. Never silently substitute a new client seed after disclosure.
    requireThat(accepted.clientSeed === body.clientSeed, 'FAIR_CLIENT_SEED_MISMATCH', 409);
    const proof = await completeSystemRequest(recorded.request);
    requireThat(proof.commitReveal?.clientSeed === body.clientSeed, 'FAIR_CLIENT_SEED_MISMATCH', 409);
    return { status: 200, body: { status: 'ready', proof } };
  }
  ...
  async function result(body, headers) {
    ...
    if (protocolOf(recorded.request) === SYSTEM_PROTOCOL) {
      if (recorded.clientSeed) {
        liveSystemBinding(recorded, room);
        return { status: 200, body: { status: 'ready', proof: await completeSystemRequest(recorded.request) } };
      }
    } else startJob(recorded.request);
  ...
  async function state(body, headers) {
    ...
    const options = { actorColor: room.actor.actorColor, botOwner: room.actor.bot && room.actor.ownsHost,
      fairDiceProtocol: roomProtocol(room) };
    ...
    if (previous?.phase === 'over' && next?.phase === 'over') {
      ...
      if (same(positionOf(previous), positionOf(next)) && previous.finishedAt === next.finishedAt) {
        ...
        return { status: 200, body: compactStateAck(room, {}, { unchanged: true }) };
      }
    }
    ...
    if (previous?.phase === 'over' && next?.phase === 'opening') {
      ...
      const committed = await store.resetFairGame(code, next, room.version, headers);
      return { status: 200, body: compactStateAck(room, committed) };
    }
    ...
    if (pending && !issuedProof && !concession) {
      return { status: 200, body: compactStateAck(room, {}, { deferred: true }) };
    }
    const committed = await store.commitState(code, next, room.version, headers);
    return { status: 200, body: compactStateAck(room, committed) };
  }
  ...
  async function handler(request, response) {
    ...
    try {
      ...
      if (request.method === 'GET' && request.url === PREFIX + '/health') {
        send(response, 200, { ok: true, protocol: FairDice.PROTOCOL, supportedProtocols: SUPPORTED_PROTOCOLS,
          chainHash: FairDice.CHAIN.hash, publicKey, pendingJobs: jobs.size });
        return;
      }
      requireThat(request.method === 'POST', 'METHOD_NOT_ALLOWED', 405);
      const routes = { [PREFIX + '/reserve']: reserve, [PREFIX + '/challenge']: challenge,
        [PREFIX + '/result']: result, [PREFIX + '/state']: state,
        [PREFIX + '/leave']: leave };
  ...
  async function resumePending() {
    ...
    for (const item of pending) {
      if (!item.cancelled && !item.consumed && !item.proof) {
        ...
        if (protocolOf(request) === SYSTEM_PROTOCOL) {
          // The initial reservation must remain unrevealed until a client
          // challenge is durable. Startup only resumes an already-fixed seed.
          const recorded = item.clientSeed ? item : await store.getRequest(request.id);
          if (!recorded.clientSeed) continue;
          void completeSystemRequest(request).catch(() => {});
        } else startJob(request);
        started += 1;
      }
    }
    return started;
