...
function proofFor(state, event, options, historical) {
  ...
  requireThat(proof && proof.protocol === PROTOCOL && Array.isArray(proof.dice) && proof.dice.length === 2
  ...
  requireThat(prior.every(item => item && item.protocol === PROTOCOL), 'fair_proof_required', 'Protected history contains an unissued roll');
...
function replayForUndo(previous, next) {
  ...
  for (const event of next.history.slice().reverse()) appendEvent(replay, event, { openingProofOnObject }, true);
  return replay;
}
...
function validateTransition(previous, next, options = {}) {
  ...
  const replay = replayForUndo(previous, next);
